...
import { Scan, Gift, Upload, Zap, DollarSign, Repeat, ArrowRight } from 'lucide-react';

export type ShoppingTask = 'vibe' | 'tradeoff' | 'gift';

interface HeroBentoProps {
  onSearch: (query: string, budget?: number, image?: string) => void;
  isSearching: boolean;
  activeTask: ShoppingTask;
  setActiveTask: (task: ShoppingTask) => void;
}

const HeroBento: React.FC<HeroBentoProps> = ({ onSearch, isSearching, activeTask, setActiveTask }) => {
        ...
        {tasks.map((task) => (
          <button
            ...
            className={`relative px-8 py-3 rounded-xl text-[11px] font-bold transition-all flex items-center gap-2.5 overflow-hidden ${
              activeTask === task.id ? 'text-white' : 'opacity-40 hover:opacity-80'
            }`}
          >
            {activeTask === task.id && (
              <motion.div
            ...
            <span className="relative z-10 tracking-widest uppercase">{task.label}</span>
          </button>
        ))}
      </div>
        ...
        {activeTask === 'vibe' && (
          ...
          >
            <div className="flex-1 flex flex-col justify-center">
              ...
              <h2 className="text-4xl font-extrabold tracking-tight mb-4">Vibe-on-a-Budget</h2>
              <p className="opacity-50 text-md max-w-sm mb-10 leading-relaxed">Reverse search luxury aesthetics to find secret value matches under your limit.</p>
              ...
              <div className="space-y-5">
                <div className="glass h-16 rounded-2xl flex items-center px-6 gap-4 border-white/5 focus-within:border-teal/40 transition-all shadow-inner group">
                  <DollarSign size={20} className="opacity-20 group-focus-within:opacity-60 transition-opacity" />
                  <input 
                    type="number" 
                ...
                <motion.button 
                  whileHover={{ scale: 1.01, boxShadow: '0 0 40px rgba(32, 226, 215, 0.4)' }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => onSearch("Visual search match", Number(vibeBudget) || undefined, image || undefined)}
                ...
                >
                  {isSearching ? <Zap size={20} className="animate-spin" /> : <><Zap size={20} /> INITIATE SCAN</>}
                </motion.button>
              </div>
            </div>
            ...
            >
              {image ? (
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.25em] opacity-30">Drop Reference Visual</span>
                </div>
              )}
          ...
          >
            <div className="relative z-10">
              <div className="flex items-center gap-5 mb-10">
                ...
                <div>
                  <h2 className="text-4xl font-extrabold tracking-tight">Trade-Off Engine</h2>
                  <p className="opacity-50 text-sm">Strip the brand premium, retain the high-tier specs.</p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <motion.button 
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => onSearch(substituteQuery, Number(tradeOffBudget) || undefined)}
        ...
        {activeTask === 'gift' && (
          ...
          >
            <div className="relative z-10">
              ...
              <h2 className="text-4xl font-extrabold tracking-tight mb-4">Gift Manifestation</h2>
              ...
              <div className="flex flex-col gap-8">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <motion.button 
                    whileHover={{ scale: 1.02, boxShadow: '0 0 40px rgba(255, 126, 95, 0.4)' }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => onSearch(giftQuery, Number(giftBudget) || undefined)}
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                  {['Nomadic Creative', 'Dark Academia', 'Solar Punk', 'Minimalist Tech'].map(tag => (
                    <button 
                      key={tag}
                      onClick={() => setGiftQuery(`A premium gift for someone into ${tag} aesthetic...`)}
